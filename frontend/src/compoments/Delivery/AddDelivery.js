...
function AddDelivery() {
  ...
  const validateForm = () => {
    ...
    // Validate phone numbers (basic validation - allow 9-15 digits)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:8070/deliveries/', formData);
        ...
        <form className="delivery-form" onSubmit={handleSubmit}>
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="submit-btn" disabled={loading}>
              <FaSave /> {loading ? 'Creating...' : 'Create Delivery'}
            </button>
          </div>
        </form>
      </div>
